...
{
    public class PetDmgDecreaseSkill : CharacterState
    {
        ControllerPetUnitInGame _unit;

        public PetDmgDecreaseSkill(ControllerPetUnitInGame unit)
        {
            _unit = unit;
        ...
        {
            return eActorState.PetDmgDecrease;
        }
        ...
        private void SkillActivate(PetSkillKey arg1, bool arg2)
        {
            if (_unit.petCache == null)
                return;
            if (Definition.PetSkillKey.DmgDecrease == _unit.petCache.tabledata.petskillKey)
            {
                if (arg2 == false)
                {
