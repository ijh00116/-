...
{
    public class PetAtkIncreaseSkill : CharacterState
    {
        ControllerPetUnitInGame _unit;



        public PetAtkIncreaseSkill(ControllerPetUnitInGame unit)
        {
            _unit = unit;
        ...
        {
            return eActorState.PetAtkIncrease;
        }
        ...
        private void SkillActivate(PetSkillKey arg1, bool arg2)
        {
            if (_unit.petCache == null)
                return;
            if (Definition.PetSkillKey.AtkIncrease == _unit.petCache.tabledata.petskillKey)
            {
                if(arg2==false)
                {
